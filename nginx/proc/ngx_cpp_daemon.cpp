...
bool CNgx_cpp_daemon::ngx_daemon()
{
    ...
    if(nullptr==pconfig)
    {
        CNgx_cpp_main::m_ngx_log.ngx_log_error_core(NGX_LOG_EMERG,0,
            "CNgx_cpp_daemon::ngx_daemon()中的CNgx_cpp_config::get_m_pconfig()失败");
        return false;
    }
    ...
    if(nDaemon)
    {
        ...
        while(1)
        {
            ...
            case -1:
                CNgx_cpp_main::m_ngx_log.ngx_log_error_core(NGX_LOG_EMERG,errno,
                    "CNgx_cpp_daemon::ngx_daemon()中的fork()失败");
                return false;
            case  0:
        ...
        if(-1 == setsid())
        {
            CNgx_cpp_main::m_ngx_log.ngx_log_error_core(NGX_LOG_WARN,errno,
                "CNgx_cpp_daemon::ngx_daemon()中的setsid()失败");
            return false;
        }
        ...
        {
            CNgx_cpp_main::m_ngx_log.ngx_log_error_core(NGX_LOG_WARN,errno,
                "CNgx_cpp_daemon::ngx_daemon()中的open()失败");
            return false;
        }
        if(-1 == dup2(m_fd,STDIN_FILENO))
        {
            CNgx_cpp_main::m_ngx_log.ngx_log_error_core(NGX_LOG_WARN,errno,
                "CNgx_cpp_daemon::ngx_daemon()中的dup2()失败");
            return false;
        }
